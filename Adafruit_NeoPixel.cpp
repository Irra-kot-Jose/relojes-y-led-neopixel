...
void Adafruit_NeoPixel::neoPixelModo(int modo)
{
  extern boolean flag;    //extern le permite obtener su valor de un fichero externo
  int y;
  ...
  switch(modo)
  {
    case 1:
      ...
      for(int i=0; i<numLEDs; i++)
      {
        ...
        deb_Black.read();               // read the button
        if(!digitalRead(REDBUTTON))  interrupt_redButton();
        if(!digitalRead(GREENBUTTON))  interrupt_greenButton();
        if(!digitalRead(YELLOWBUTTON))  interrupt_yellowButton();
      }
    ...
    case 2:
      ...
      for(int i=0; i<(numLEDs/2); i++)
      {
        ...
        setPixelColor(i, Color(R,G,B));
        setPixelColor((numLEDs-1)-i, Color(R,G,B)); 
        show(); 
        delay(250);
        }
        deb_Black.read();
    ...
    case 3:
      fill(Color(R,G,B),0,numLEDs);   //Selecciona un color; que empiece del led posicion 0; cantidad de leds que hay. ENCIENDE TODA LA BARRA
      show();
      deb_Black.read();
      if(!digitalRead(REDBUTTON))  interrupt_redButton();
      if(!digitalRead(GREENBUTTON))  interrupt_greenButton();
    ...
    case 4:
      ...
      while(y <= numLEDs && !flag){
        for(int i=0; i<(numLEDs-y); i++)
        {
          if (flag){
            break;
          }else{
        ...
        for(int i=0; i<25; i++)
        {
          if (flag){
            ...
            fill(Color(0,0,0),0,numLEDs);
            show();
            delay(50);
            fill(Color(R,G,B),0,numLEDs);
            show();
            delay(50);
          }
          deb_Black.read();
